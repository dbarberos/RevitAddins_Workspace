...
- **Modeless Revit API Interaction**: Para interactuar con herramientas de selección interactiva (`PickObjects`) desde ventanas WPF Modeless, se debe usar obligatoriamente `IExternalEventHandler`. Para dar feedback visual, pasar la selección actual como `IList<Reference>` a la herramienta de selección para que Revit mantenga el resaltado azul.
- **Multi-Scope State Synchronization**: Al manejar múltiples vistas o filtros sobre un conjunto de datos (ej. un explorador de árbol con filtros activos), el estado persistente de selecciones NO DEBE sobrescribirse únicamente con los elementos visibles actuales. Se debe realizar una "Unión Inteligente" (mantener IDs del estado persistente que no formen parte del alcance actual + elementos seleccionados en el alcance actual) para evitar pérdidas de selección silenciosas al cambiar de vista o aplicar búsquedas.
- **Dynamic Hierarchy (Recursive Building)**: Para soportar múltiples niveles de agrupación (ej. por Fase, por Nivel, por Workset) donde el orden de anidamiento dependa del usuario, evitar estructuras `if-else` anidadas. Implementar un método recursivo que consuma una pila o lista de agrupadores activos. Esto permite un escalado infinito de niveles de organización sin aumentar la complejidad del código.
