...
- **Threading & Modeless WPF/WebView2 (`revit-api-core`, `revit-async-operations`, `revit-api-resilience`)**: Modifying the document or starting transactions inside modeless/floating WPF views or viewmodels (RelayCommands) MUST be done through the Revit API context asynchronously via `Revit.Async` (`await RevitTask.RunAsync(...)`), `IExternalEventHandler` or by queuing actions in the native `AsyncTaskDispatcher`. Directly starting transactions in UI/background threads is strictly prohibited.
- **Failure Handling & Telemetry (`revit-api-resilience`)**: When running batch operations or automated edits, register a `WarningSwallower` to intercept and delete non-fatal warnings without displaying dialogs. Never suppress `FailureSeverity.Error` or `FailureSeverity.DocumentCorruption`. When logging exceptions, the agent MUST sanitize paths (scrub user directories via `TelemetryLogger`) to prevent PII leaks.
- **Dynamic Model Update (DMU) (`revit-api-resilience`)**: Register `IUpdater` classes exclusively inside `IExternalApplication.OnStartup()` after checking `UpdaterRegistry.IsUpdaterRegistered()`. Always enforce recursion prevention (anti-loops) and explicitly unregister updaters on `OnShutdown()` to prevent host memory corruption.
...
- **WPF UI Design & Performance (`revit-addin-gui-design`)**: For controls displaying 1000+ items (ListView, TreeView, DataGrid), WPF virtualization is mandatory (never wrap in a ScrollViewer, never disable content scroll). UI setups must follow premium card-based design systems with smooth animations (such as FilterPlus SwitchStyle) and keep vertical scrollbars physically left-sided via inverted margins under FlowDirection. Always apply the 100,000 element safety limit check to prevent UI freezes on linked documents. All Window styles, templates, and converters MUST be declared inline within `<Window.Resources>` (the FilterPlus pattern); NEVER import external ResourceDictionaries via `pack://application:,,,/` inside `ResourceDictionary.Source` as Revit's unmanaged host lacks a standard WPF Application lifecycle, triggering `XamlParseException`. Configure `<DefaultItemExcludes>` in `.csproj` to prevent MSBuild from crawling package directories and contaminating BAML resources. In interactive hierarchical TreeViews with tri-state selection, negative regex/exclusion patterns (such as `(?!` or `(?<!`) must be evaluated strictly on leaf nodes bottom-up (using `RefreshState()`), never cascading top-down from container nodes, to prevent false positive 100% selection.
...
- **App Store Bundles & Autoloader (`revit-appstore-bundle`)**: In `PackageContents.xml`, the `SeriesMin` and `SeriesMax` attributes MUST strictly declare the `"R"` prefix (`R2023`, `R2024`, `R2025`, `R2026`, `R2027`). Omitting the `"R"` causes Revit's native Autoloader to evaluate string comparison (`"R202X" <= "202X"`) as false and silently discard the bundle on startup across all versions. In multi-version codebases supporting Revit 2023 alongside Revit 2024+, use `#if REVIT2024_OR_GREATER` to bridge 64-bit `ElementId.Value` with 32-bit `ElementId.IntegerValue`. All add-in entry points must register an `AppDomain.CurrentDomain.AssemblyResolve` hook in `OnStartup()` to ensure dependency resolution in .NET 8 (Revit 2025+). In accordance with Autodesk App Store single-command guidelines, ribbon panels MUST load by default under the native **Add-Ins** (`Complementos`) tab (`Application.CreatePanel("[AppName]")`); creating custom tabs by default is strictly prohibited and causes immediate store rejection. When generating store bundles, `build-bundle.ps1` MUST force clean multi-version recompilation (`-Rebuild`) with `/p:DeployAddin=false` to prevent packaging stale binaries from prior months.
...
| `revit-addin-gui-design` | `.agents/skills/revit-addin-gui-design/` | Modern UI/UX systems, WPF virtualization, Fluent layouts, and premium FilterPlus card-based styles. |
