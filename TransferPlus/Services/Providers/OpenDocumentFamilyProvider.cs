...
public class OpenDocumentFamilyProvider : IFamilyProvider
{
    ...
    public Task<bool> TransferFamilyAsync(FamilyItemModel familyItem, Document destinationDoc, string? overrideFamilyName = null, IDictionary<string, string>? symbolRenameMap = null, CancellationToken cancellationToken = default)
    {
        ...
        if (familyItem.NativeFamily is Family sourceFamily)
        {
            ...
            bool success = _familyRevitService.TryTransferInMemoryFamily(_sourceDoc, sourceFamily, destinationDoc, out _, targetSymbolNames, overrideFamilyName, symbolRenameMap);
            if (success)
            {
